...
public class ConnectedTexturesCompact {
    ...
    private static BakedQuad makeSpriteQuadCompact(BakedQuad quad, TextureAtlasSprite sprite, int side, int x1, int y1, int x2, int y2) {
        ...
        PositionUv[] apositionuv = new PositionUv[4];

        for (int i = 0; i < apositionuv.length; i++) {
            apositionuv[i] = fixVertexCompact(quad.position(i), quad.packedUV(i), textureatlassprite, sprite, side, x1, y1, x2, y2);
        }

        return quad.makeCopy(apositionuv, sprite);
    }

    private static PositionUv fixVertexCompact(
        Vector3fc pos, long pUV, TextureAtlasSprite spriteFrom, TextureAtlasSprite spriteTo, int side, int x1, int y1, int x2, int y2
    ) {
        float f = UVPair.unpackU(pUV);
        float f1 = UVPair.unpackV(pUV);
        double d0 = spriteFrom.getUnInterpolatedU16(f);
        double d1 = spriteFrom.getUnInterpolatedV16(f1);
        float f2 = pos.x();
        float f3 = pos.y();
        float f4 = pos.z();
        float f5;
        ...
        float f7 = 0.0F;
        float f8 = 16.0F * (1.0F - f7);
        float f9 = 16.0F * (1.0F - f7);
        if (d0 < x1) {
            f5 = (float)(f5 + (x1 - d0) / f8);
            d0 = x1;
        }

        if (d0 > x2) {
            f5 = (float)(f5 - (d0 - x2) / f8);
            d0 = x2;
        }

        if (d1 < y1) {
            f6 = (float)(f6 + (y1 - d1) / f9);
            d1 = y1;
        }

        if (d1 > y2) {
            f6 = (float)(f6 - (d1 - y2) / f9);
            d1 = y2;
        }
        ...
        float f10 = spriteTo.getInterpolatedU16(d0);
        float f11 = spriteTo.getInterpolatedV16(d1);
        long i = UVPair.pack(f10, f11);
        Vector3f vector3f = new Vector3f(f2, f3, f4);
        return new PositionUv(vector3f, i);
    }
