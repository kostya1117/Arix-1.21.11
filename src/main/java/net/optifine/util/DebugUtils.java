...
public class DebugUtils {
    ...
    public static String getMatrix4(FloatBuffer fb) {
        fb.clear();
        fb.get(floatArray16);
        fb.clear();
        return getMatrix4(floatArray16, "\n");
    ...
    public static void debugVboMemory(SectionRenderDispatcher.RenderSection[] renderChunks) {
        if (TimedEvent.isActive("DbgVbos", 3000L)) {
            ...
            for (int i1 = 0; i1 < renderChunks.length; i1++) {
                ...
                for (ChunkSectionLayer chunksectionlayer : SectionRenderDispatcher.BLOCK_RENDER_LAYERS) {
                    GpuBuffer gpubuffer = sectionmesh.getBuffers(chunksectionlayer).getVertexBuffer();
                    if (sectionrenderdispatcher$rendersection.getSectionMesh().isLayerUsed(chunksectionlayer)) {
                        l++;
                    }
                }
    ...
    public static void debugTextures() {
        ...
        long i = 0L;
        Collection<Identifier> collection = texturemanager.getTextureLocations();
        List<Identifier> list = new ArrayList<>(collection);
    ...
    public static void waitDebug(int sec) {
    ...
    public static void waitRenderDoc() {
        System.out.print("Waiting for RenderDoc: ");

        for (int i = 1; i < 3600; i++) {
            HMODULE hmodule = Kernel32.INSTANCE.GetModuleHandle("renderdoc.dll");
            if (hmodule != null) {
                break;
            }
