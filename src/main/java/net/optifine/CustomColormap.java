...
public class CustomColormap implements CustomColors.IColorizer {
    ...
    private int getColorVanilla(Biome biome, BlockPos blockPos) {
        double d0 = Mth.clamp(biome.getBaseTemperature(), 0.0F, 1.0F);
        double d1 = Mth.clamp(BiomeUtils.getDownfall(biome), 0.0F, 1.0F);
        d1 *= d0;
        int i = (int)((1.0 - d0) * (this.width - 1));
        int j = (int)((1.0 - d1) * (this.height - 1));
    ...
    private int getColorGrid(Biome biome, BlockPos blockPos) {
        ...
        int j = i;
        int k = blockPos.getY() - this.yOffset;
        if (this.yVariance > 0) {
            int l = blockPos.getX() << 16 + blockPos.getZ();
            int i1 = Config.intHash(l);
            int j1 = this.yVariance * 2 + 1;
            int k1 = (i1 & 0xFF) % j1 - this.yVariance;
            k += k1;
        }

        return this.getColor(j, k);
    }
