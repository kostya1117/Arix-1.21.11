...
public class NbtTagValue {
    ...
    private static String getNbtString(Tag nbtBase, int format) {
        ...
        if (!(nbtBase instanceof StringTag stringtag)) {
            if (nbtBase instanceof IntTag inttag) {
                return format == 1 ? "#" + StrUtils.fillLeft(Integer.toHexString(inttag.value()), 6, '0') : Integer.toString(inttag.value());
            } else if (nbtBase instanceof ByteTag bytetag) {
                return Byte.toString(bytetag.value());
            } else if (nbtBase instanceof ShortTag shorttag) {
                return Short.toString(shorttag.value());
            } else if (nbtBase instanceof LongTag longtag) {
                return Long.toString(longtag.value());
            } else if (nbtBase instanceof FloatTag floattag) {
                return Float.toString(floattag.value());
            } else {
                return nbtBase instanceof DoubleTag doubletag ? Double.toString(doubletag.value()) : nbtBase.toString();
            }
        } else {
            String s = stringtag.value();
    ...
    private static int getNbtInt(Tag nbtBase, int defVal) {
        if (nbtBase == null) {
            ...
            return bytetag.intValue();
        } else if (nbtBase instanceof ShortTag shorttag) {
            return shorttag.intValue();
        } else if (nbtBase instanceof LongTag longtag) {
            return longtag.intValue();
        } else if (nbtBase instanceof FloatTag floattag) {
            return floattag.intValue();
        } else {
            return nbtBase instanceof DoubleTag doubletag ? doubletag.intValue() : defVal;
        }
    }
    ...
    public String toString() {
        return ArrayUtils.arrayToString(this.path, ".") + " = " + this.value;
    }
}
