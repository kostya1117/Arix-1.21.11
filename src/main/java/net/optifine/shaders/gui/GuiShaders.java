...
public class GuiShaders extends GuiScreenOF {
    ...
    public void updateButtons() {
        ...
        for (AbstractWidget abstractwidget : this.getButtonList()) {
            if (abstractwidget instanceof GuiButtonOF guibuttonof
                ...
                && guibuttonof.id != 202
                && guibuttonof.id != 210
                && guibuttonof.id != EnumShaderOption.ANTIALIASING.ordinal()) {
            ...
            if (guiElement instanceof GuiButtonEnumShaderOption guibuttonenumshaderoption) {
                ...
                switch (enumshaderoption) {
                    ...
                    case SHADOW_CLIP_FRUSTRUM:
                        ...
                        guibuttonenumshaderoption.setMessage(
                            "ShadowClipFrustrum: " + toStringOnOff(Shaders.configShadowClipFrustrum, Boolean.valueOf(enumshaderoption.getValueDefault()))
                        );
                        ShadersTex.updateTextureMinMagFilter();
                }
    ...
    public void render(GuiGraphics graphicsIn, int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(graphicsIn, this.fontRenderer, this.title, this.width / 2, 15, -1);
        String s = "OpenGL: " + Shaders.glVersionString + ", " + Shaders.glVendorString + ", " + Shaders.glRendererString;
        int i = this.fontRenderer.width(s);
        if (i < this.width - 5) {
            drawCenteredString(graphicsIn, this.fontRenderer, s, this.width / 2, this.height - 40, -6250336);
        } else {
            drawString(graphicsIn, this.fontRenderer, s, 5, this.height - 40, -6250336);
        }
    ...
    public static String toStringOnOff(boolean value, boolean defValue) {
        String s = value ? Lang.getOn() : Lang.getOff();
        String s1 = "";
        if (value != defValue) {
            s1 = value ? "\u00a7a" : "\u00a7c";
        }
    ...
    public static String toStringAa(int value, int valueDef) {
        String s = Lang.getOff();
        if (value == 2) {
            s = "FXAA 2x";
        }
    ...
    public static String toStringQuality(float val, float valDef) {
        String s = val != valDef ? "\u00a7a" : "";
        return s + toStringValue(val, QUALITY_MULTIPLIERS, QUALITY_MULTIPLIER_NAMES);
    }

    public static String toStringHandDepth(float val, float valDef) {
        String s = val != valDef ? "\u00a7a" : "";
        return s + toStringValue(val, HAND_DEPTH_VALUES, HAND_DEPTH_NAMES);
    }
