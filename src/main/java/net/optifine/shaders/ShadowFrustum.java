...
public class ShadowFrustum extends Frustum {
    public ShadowFrustum(Matrix4f matrixIn, Matrix4f projectionIn) {
        super(matrixIn, projectionIn, true);
    ...
    private void extendForShadows(Matrix4f matrixIn, Matrix4f projectionIn) {
        ...
        if (clientlevel != null) {
            ...
            vector4f1.normalize();
            float f = 0.0F;
            float f1 = WorldUtils.getCelestialAngleRadians(f);
            ...
            Vector4f vector4f5 = this.planes[0];
            Vector4f vector4f6 = this.planes[1];
            Vector4f vector4f7 = this.planes[2];
            Vector4f vector4f8 = this.planes[3];
            Vector4f vector4f9 = this.planes[4];
            Vector4f vector4f10 = this.planes[5];
            vector4f5.normalize();
            vector4f6.normalize();
            vector4f7.normalize();
            vector4f8.normalize();
            ...
            float f17 = (int)Config.getMinecraft().player.getEyeY();
            float f18 = Config.limit(f17 - i, 0.0F, f11);
            float f19 = Config.limit(j - f17, 0.0F, f11);
            float f20 = 0.0F;
            float f21 = 0.0F;
            if (f9 < 0.0F || f10 < 0.0F) {
                vector4f10.add(0.0F, 0.0F, 0.0F, f11);
    ...
    private float rotateDotPlus(Vector4f vecFrustum, Vector4f vecSun, int angleDeg, Vector4f vecRot) {
        ...
        Quaternionf quaternionf = MathUtils.rotationDegrees(vector3f, angleDeg);
        float f = 0.0F;
