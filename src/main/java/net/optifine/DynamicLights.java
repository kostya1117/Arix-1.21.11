...
public class DynamicLights {
    ...
    private static void initialize() {
        ...
        for (int i = 0; i < astring.length; i++) {
            ...
            try {
                Identifier identifier = new Identifier(s, "optifine/dynamic_lights.properties");
                InputStream inputstream = Config.getResourceStream(identifier);
                loadModConfiguration(inputstream, identifier.toString(), s);
    ...
    public static int getCombinedLight(Entity entity, int combinedLight) {
        ...
        if (entity == Config.getMinecraft().player) {
            double d1 = getLightLevel(entity);
            d0 = Math.max(d0, d1);
        }
    ...
    public static int getLightLevel(Entity entity) {
        if (entity == Config.getMinecraft().getCameraEntity() && !Config.isDynamicHandLight()) {
            ...
            if (entity instanceof AbstractHurtingProjectile) {
                ...
                return magmacube.squish > 0.6 ? 13 : 8;
            } else if (entity instanceof Creeper creeper && creeper.getSwelling(0.0F) > 0.001) {
                return 15;
            } else if (entity instanceof GlowSquid glowsquid) {
                return (int)Mth.clampedLerp(0.0F, 11.0F, 1.0F - glowsquid.getDarkTicksRemaining() / 10.0F);
