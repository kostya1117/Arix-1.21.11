...
public class CustomSkyLayer {
    ...
    public GpuTextureView gpuTexture;
    ...
    private int parseTime(String str) {
        ...
        if (i >= 0 && i <= 23 && j >= 0 && j <= 59) {
            ...
            return i * 1000 + (int)(j / 60.0 * 1000.0);
        } else {
            Config.warn("Invalid time: " + str);
            return -1;
        }
    ...
    private float getWeatherBrightness(float rainStrength, float thunderStrength) {
        ...
        return NumUtils.limit(f3, 0.0F, 1.0F);
    }
    ...
    private void renderSide(PoseStack matrixStackIn, Tesselator tess, int side) {
        ...
        BufferUploader.draw(RenderPipelines.CELESTIAL, bufferbuilder.buildOrThrow(), () -> "SkyLayerSide:" + side, this.gpuTexture, FilterMode.NEAREST);
    }
    ...
    private void addVertex(Matrix4f matrix4f, BufferBuilder buffer, float x, float y, float z, float u, float v) {
        ...
        buffer.addVertex(f, f1, f2).setUv(u, v).setColor(-1);
    }
    ...
    public boolean isActive(Level world, int timeOfDay) {
        ...
        if (this.days != null) {
            ...
            long j = i - this.startFadeIn;

            while (j < 0L) {
                j += 24000 * this.daysLoop;
            }
