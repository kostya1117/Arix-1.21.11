...
public class MultiTextureRenderer {
    ...
    private static void draw(VertexFormat.Mode drawMode, int indexTypeIn, SpriteRenderData srd) {
        ...
        if (shaders) {
            int i = textureatlassprite.spriteNormal != null ? textureatlassprite.spriteNormal.getSpriteTextureId() : 0;
            int j = textureatlassprite.spriteSpecular != null ? textureatlassprite.spriteSpecular.getSpriteTextureId() : 0;
        ...
        for (int i1 = 0; i1 < aint.length; i1++) {
            bufferPositions.put(drawMode.indexCount(aint[i1]) * l);
        }
        ...
        bufferCounts.flip();
        GlStateManager.glMultiDrawElements(GlConst.toGl(drawMode), bufferCounts, indexTypeIn, bufferPositions);
    }
