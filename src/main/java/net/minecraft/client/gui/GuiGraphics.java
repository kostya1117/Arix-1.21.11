...
public class GuiGraphics implements IForgeGuiGraphics {
    ...
    public void blit(Identifier locationIn, int startX, int startY, int endX, int endY, float minU, float maxU, float minV, float maxV, int color) {
        this.innerBlit(RenderPipelines.GUI_TEXTURED, locationIn, startX, endX, startY, endY, minU, maxU, minV, maxV, color);
    }
    ...
    private void innerBlit(
        ...
        if (Config.isCustomGuis()) {
            p_458466_ = CustomGuis.getTextureLocation(p_458466_);
        }
    ...
    public void setTooltipForNextFrame(
        ...
        p_456997_.ifPresent(compIn -> list.add(list.isEmpty() ? 0 : 1, ClientTooltipComponent.create(compIn)));
        if (Reflector.ForgeHooksClient_gatherTooltipComponents7.exists()) {
            List list1 = (List)Reflector.ForgeHooksClient_gatherTooltipComponents7
                .call(this.tooltipStack, p_405939_, p_456997_, p_409519_, this.guiWidth(), this.guiHeight(), p_408450_);
            list.clear();
            list.addAll(list1);
        }
    ...
    public void setComponentTooltipForNextFrame(Font p_406239_, List<Component> p_410275_, int p_410594_, int p_409343_) {
        if (Reflector.ForgeHooksClient_gatherTooltipComponents6.exists()) {
            List<ClientTooltipComponent> list = (List<ClientTooltipComponent>)Reflector.ForgeHooksClient_gatherTooltipComponents6
                .call(this.tooltipStack, p_410275_, p_410594_, this.guiWidth(), this.guiHeight(), p_406239_);
            this.setTooltipForNextFrameInternal(p_406239_, list, p_410594_, p_409343_, DefaultTooltipPositioner.INSTANCE, null, false);
        } else {
            this.setComponentTooltipForNextFrame(p_406239_, p_410275_, p_410594_, p_409343_, (Identifier)null);
        }
    }
    ...
    public void renderComponentTooltip(Font font, List<? extends FormattedText> tooltips, int mouseX, int mouseY, ItemStack stack) {
        ...
        this.setTooltipForNextFrameInternal(font, list, mouseX, mouseY, DefaultTooltipPositioner.INSTANCE, null, false);
        this.tooltipStack = ItemStack.EMPTY;
    }
    ...
    private void setTooltipForNextFrameInternal(
        ...
        if (!p_409928_.isEmpty() && (this.deferredTooltip == null || p_410618_)) {
            if (Reflector.ForgeHooksClient.exists()) {
                ItemStack itemstack = this.tooltipStack;
                this.deferredTooltip = () -> this.renderTooltip(p_406605_, p_409928_, p_407794_, p_408784_, p_408798_, p_451079_, itemstack);
            }
    ...
    public void renderTooltip(
        ...
        this.renderTooltip(p_283128_, p_282716_, p_283678_, p_281696_, p_409451_, p_459396_, ItemStack.EMPTY);
    }

    public void renderTooltip(
        Font fontIn, List<ClientTooltipComponent> compsIn, int x, int y, ClientTooltipPositioner posIn, @Nullable Identifier locIn, ItemStack itemstack
    ) {
        Object object = null;
        if (Reflector.ForgeHooksClient_onRenderTooltipPre.exists()) {
            object = Reflector.ForgeHooksClient_onRenderTooltipPre
                .call(itemstack, this, x, y, this.guiWidth(), this.guiHeight(), compsIn, fontIn, posIn, locIn);
            if (object == null) {
                return;
            }
        }
        ...
        int i = 0;
        int j = compsIn.size() == 1 ? -2 : 0;

        for (ClientTooltipComponent clienttooltipcomponent : compsIn) {
            if (object != null) {
                fontIn = (Font)Reflector.call(object, Reflector.RenderTooltipEvent_getFont);
            }

            int k = clienttooltipcomponent.getWidth(fontIn);
            if (k > i) {
                i = k;
            }

            j += clienttooltipcomponent.getHeight(fontIn);
        }

        int l1 = i;
        int i2 = j;
        if (object != null) {
            x = Reflector.callInt(object, Reflector.RenderTooltipEvent_getX);
            y = Reflector.callInt(object, Reflector.RenderTooltipEvent_getY);
        }

        Vector2ic vector2ic = posIn.positionTooltip(this.guiWidth(), this.guiHeight(), x, y, i, j);
        int l = vector2ic.x();
        int i1 = vector2ic.y();
        this.pose.pushMatrix();
        if (Reflector.RenderTooltipEvent_Pre_getBackground.exists()) {
            locIn = (Identifier)Reflector.call(object, Reflector.RenderTooltipEvent_Pre_getBackground);
        }

        TooltipRenderUtil.renderTooltipBackground(this, l, i1, i, j, locIn);
        int j1 = i1;

        for (int k1 = 0; k1 < compsIn.size(); k1++) {
            ClientTooltipComponent clienttooltipcomponent1 = compsIn.get(k1);
            clienttooltipcomponent1.renderText(this, fontIn, l, j1);
            j1 += clienttooltipcomponent1.getHeight(fontIn) + (k1 == 0 ? 2 : 0);
        }

        j1 = i1;

        for (int j2 = 0; j2 < compsIn.size(); j2++) {
            ClientTooltipComponent clienttooltipcomponent2 = compsIn.get(j2);
            clienttooltipcomponent2.renderImage(fontIn, l, j1, l1, i2, this);
            j1 += clienttooltipcomponent2.getHeight(fontIn) + (j2 == 0 ? 2 : 0);
        }
    ...
    private void renderItemBar(ItemStack p_367359_, int p_362139_, int p_368464_) {
        if (p_367359_.isBarVisible()) {
            ...
            int k = p_367359_.getBarColor();
            if (Config.isCustomColors()) {
                float f = p_367359_.getDamageValue();
                float f1 = p_367359_.getMaxDamage();
                float f2 = Math.max(0.0F, (f1 - f) / f1);
                k = CustomColors.getDurabilityColor(f2, k);
            }

            this.fill(RenderPipelines.GUI, i, j, i + p_367359_.getBarWidth(), j + 1, ARGB.opaque(k));
        }
    }
        ...
        if (p_282156_ != null && p_282156_.getHoverEvent() != null) {
            switch (p_282156_.getHoverEvent()) {
                ...
                case HoverEvent.ShowEntity(HoverEvent.EntityTooltipInfo hoverevent$entitytooltipinfo):
                    HoverEvent.EntityTooltipInfo hoverevent$entitytooltipinfo1 = hoverevent$entitytooltipinfo;
                    if (this.minecraft.options.advancedItemTooltips) {
                        this.setComponentTooltipForNextFrame(p_282584_, hoverevent$entitytooltipinfo1.getTooltipLines(), p_283623_, p_282114_);
                    }
                    break;
    ...
    public enum HoveredTextEffects {
        ...
        HoveredTextEffects(final boolean p_460939_, final boolean p_460474_) {
