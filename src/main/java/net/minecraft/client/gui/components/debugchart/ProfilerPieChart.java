...
public class ProfilerPieChart {
    ...
    public void setPieChartResults( ProfileResults p_369896_) {
    ...
    public void render(GuiGraphics p_365682_) {
        if (this.profilerPieChartResults != null) {
            ...
            int k = i + 105;
            int l = list.size() * 9;
            if (!Config.equals(this.lastRoot, resultfield.name)) {
                this.lastRoot = resultfield.name;
                this.lastPosY = 0;
            }

            this.lastPosY = Math.max(this.lastPosY, l);
            l = this.lastPosY;
