...
public class GuiRenderer implements AutoCloseable {
    ...
    public GuiRenderer(
        ...
        Reflector.ForgeHooksClient_onRegisterPictureInPictureRenderers.call(p_410565_, p_410441_, builder);
    ...
    private void prepare() {
        ...
        for (int i = 0; i < this.renderState.frameListeners.size(); i++) {
            this.renderState.frameListeners.get(i).addToMeshBegin();
        }

        this.addElementsToMeshes(GuiRenderState.TraverseRange.BEFORE_BLUR);

        for (int j = 0; j < this.renderState.frameListeners.size(); j++) {
            this.renderState.frameListeners.get(j).addToMeshEnd();
        }

        this.renderState.frameListeners.clear();
    ...
    private void prepareText() {
        this.renderState.forEachText(state2In -> {
            final Matrix3x2fc matrix3x2fc = state2In.pose;
            final ScreenRectangle screenrectangle = state2In.scissor;
            state2In.ensurePrepared().visit(new Font.GlyphVisitor() {
                @Override
    ...
    private void prepareItemElements() {
        if (!this.renderState.getItemModelIdentities().isEmpty()) {
            ...
            this.renderState
                .forEachItem(
                    state2In -> {
                        if (state2In.oversizedItemBounds() != null) {
                            mutableboolean1.setTrue();
                        } else {
                            TrackingItemStackRenderState trackingitemstackrenderstate = state2In.itemStackRenderState();
                            ...
                            if (guirenderer$atlasposition == null
                                ...
                                if (flag || this.itemAtlasY + j <= k) {
                                    ...
                                    this.submitBlitFromItemAtlas(state2In, f, f1, j, k);
                                    if (flag) {
                                        guirenderer$atlasposition.lastAnimatedOnFrame = this.frameNumber;
                                    } else {
                                        this.atlasPositions
                                            .put(
                                                state2In.itemStackRenderState().getModelIdentity(),
                                ...
                                this.submitBlitFromItemAtlas(state2In, guirenderer$atlasposition.u, guirenderer$atlasposition.v, j, k);
                            }
                        }
                    }
                );
                        ...
                        state2In -> {
                            if (state2In.oversizedItemBounds() != null) {
                                ...
                                OversizedItemRenderer oversizeditemrenderer = this.oversizedItemRenderers
                                    .computeIfAbsent(trackingitemstackrenderstate.getModelIdentity(), key2In -> new OversizedItemRenderer(this.bufferSource));
                                ScreenRectangle screenrectangle = state2In.oversizedItemBounds();
    ...
    private void preparePictureInPicture() {
        ...
        this.renderState.forEachPictureInPicture(state2In -> this.preparePictureInPictureState(state2In, i));
    }
    ...
    private int calculateAtlasSizeInPixels(int p_407739_) {
        ...
        return Math.clamp(Mth.smallestEncompassingPowerOfTwo(i1 * p_407739_), 512, MAXIMUM_ITEM_ATLAS_SIZE);
    }
    ...
    private void recordMesh(BufferBuilder p_406593_, RenderPipeline p_410453_, TextureSetup p_408583_, @Nullable ScreenRectangle p_409159_) {
        ...
        if (meshdata != null) {
            ...
            for (int i = 0; i < this.renderState.frameListeners.size(); i++) {
                this.renderState.frameListeners.get(i).meshToDraw((GuiRenderer.MeshToDraw)this.meshesToDraw.getLast());
            }
        }
    }
    ...
    public void addMeshesToDraw(List<GuiRenderer.MeshToDraw> meshes) {
        this.meshesToDraw.addAll(meshes);
    }
    ...
    public static boolean isRenderGui() {
        return renderGui;
    }
    ...
    public record MeshToDraw(MeshData mesh, RenderPipeline pipeline, TextureSetup textureSetup, ScreenRectangle scissorArea) implements AutoCloseable {
        @Override
        public void close() {
            this.mesh.close();
        }
