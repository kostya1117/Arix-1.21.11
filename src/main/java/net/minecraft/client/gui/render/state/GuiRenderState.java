...
public class GuiRenderState {
    ...
    public final List<GuiRenderListener> frameListeners = new ArrayList<>();
    ...
    private void navigateToAboveHighestElementWithIntersectingBounds(ScreenRectangle p_408085_) {
        GuiRenderState.Node guirenderstate$node = (GuiRenderState.Node)this.strata.getLast();
    ...
    public void forEachElement(Consumer<GuiElementRenderState> p_428294_, GuiRenderState.TraverseRange p_406592_) {
        this.traverse(node2In -> {
            if (node2In.elementStates != null) {
                for (GuiElementRenderState guielementrenderstate : node2In.elementStates) {
                    p_428294_.accept(guielementrenderstate);
                }
            }

            if (node2In.glyphStates != null) {
                for (GuiElementRenderState guielementrenderstate1 : node2In.glyphStates) {
    ...
    public void forEachItem(Consumer<GuiItemRenderState> p_407719_) {
        ...
        this.traverse(node2In -> {
            if (node2In.itemStates != null) {
                this.current = node2In;

                for (GuiItemRenderState guiitemrenderstate : node2In.itemStates) {
    ...
    public void forEachText(Consumer<GuiTextRenderState> p_410457_) {
        GuiRenderState.Node guirenderstate$node = this.current;
        this.traverse(node2In -> {
            if (node2In.textStates != null) {
                for (GuiTextRenderState guitextrenderstate : node2In.textStates) {
                    this.current = node2In;
    ...
    public void forEachPictureInPicture(Consumer<PictureInPictureRenderState> p_407751_) {
        GuiRenderState.Node guirenderstate$node = this.current;
        this.traverse(node2In -> {
            if (node2In.picturesInPictureStates != null) {
                this.current = node2In;

                for (PictureInPictureRenderState pictureinpicturerenderstate : node2In.picturesInPictureStates) {
        ...
        this.traverse(node2In -> {
            if (node2In.elementStates != null) {
                if (SharedConstants.DEBUG_SHUFFLE_UI_RENDERING_ORDER) {
                    Collections.shuffle(node2In.elementStates);
                }

                node2In.elementStates.sort(p_408774_);
            }
        }, GuiRenderState.TraverseRange.ALL);
    }
    ...
    private void traverse(Consumer<GuiRenderState.Node> p_406857_, GuiRenderState.TraverseRange p_405917_) {
        ...
        for (int k = i; k < j; k++) {
            for (int l = 0; l < this.frameListeners.size(); l++) {
                this.frameListeners.get(l).traverseStratumBegin(k);
            }

            GuiRenderState.Node guirenderstate$node = this.strata.get(k);
            this.traverse(guirenderstate$node, p_406857_);

            for (int i1 = 0; i1 < this.frameListeners.size(); i1++) {
                this.frameListeners.get(i).traverseStratumEnd(k);
            }
        }
    }
    ...
    public int getStratum() {
        return this.strata.size() - 1;
    }

    public void addFrameListener(GuiRenderListener listener) {
        this.frameListeners.add(listener);
    }

    public void removeFrameListener(GuiRenderListener listener) {
        this.frameListeners.remove(listener);
    }

    @Override
    public String toString() {
        int i = Math.min(this.firstStratumAfterBlur, this.strata.size());
        return "strata: " + this.strata.size() + ", preBlur: " + i + ", postBlur: " + (this.strata.size() - i);
    }
    ...
    static class Node {
        ...
        public String toString() {
            ...
            if (this.elementStates != null) {
                stringbuilder.append("elems: " + this.elementStates.size() + ", ");
            }

            if (this.glyphStates != null) {
                stringbuilder.append("glyphs: " + this.glyphStates.size() + ", ");
            }

            if (this.itemStates != null) {
                stringbuilder.append("items: " + this.itemStates.size() + ", ");
            }

            if (this.textStates != null) {
                stringbuilder.append("texts: " + this.textStates.size() + ", ");
            }

            if (this.picturesInPictureStates != null) {
                stringbuilder.append("pips: " + this.picturesInPictureStates.size() + ", ");
            }

            if (this.up != null) {
                stringbuilder.append("up: (" + this.up + "), ");
            }

            return StrUtils.removeSuffix(stringbuilder.toString(), ", ");
        }
    }
