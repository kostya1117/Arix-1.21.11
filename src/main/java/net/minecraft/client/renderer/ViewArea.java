...
public class ViewArea {
    ...
    public SectionRenderDispatcher.@Nullable RenderSection getRenderSectionAt(BlockPos p_299271_) {
    ...
    private void updateVboRegion(SectionRenderDispatcher.RenderSection renderChunk) {
        ...
        if (avboregion == null) {
            avboregion = new VboRegion[achunksectionlayer.length];

            for (int k = 0; k < achunksectionlayer.length; k++) {
                if (RegionLayerRenderer.canRenderLayer(achunksectionlayer[k])) {
                    avboregion[k] = new VboRegion(chunkpos, achunksectionlayer[k]);
                }
            }
        ...
        renderChunk.setVboRegions(avboregion);
    }
    ...
    public int getHighestUsedChunkIndex(int chunkX, int minChunkIndex, int chunkZ) {
        ...
        for (int i = this.sectionGridSizeY - 1; i >= minChunkIndex; i--) {
            ...
            if (!sectionrenderdispatcher$rendersection.getSectionMesh().isEmpty()) {
                return i;
            }
        }
