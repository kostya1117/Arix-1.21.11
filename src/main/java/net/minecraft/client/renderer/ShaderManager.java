...
public class ShaderManager extends SimplePreparableReloadListener<ShaderManager.Configs> implements AutoCloseable {
    ...
    protected ShaderManager.Configs prepare(ResourceManager p_363890_, ProfilerFiller p_362646_) {
        ...
        for (int i = 0; i < astring.length; i++) {
            ...
            for (int j = 0; j < astring1.length; j++) {
                ...
                Identifier identifier = Identifier.withDefaultNamespace(s2);
                Optional<Resource> optional = Config.getResourceSafe(identifier);
                if (optional.isEmpty()) {
                    Config.warn("Resource not found: " + identifier);
                } else {
                    map.put(identifier, optional.get());
                }
            }
        }
        ...
        for (Entry<Identifier, Resource> entry : map.entrySet()) {
            ...
            ShaderType shadertype = ShaderType.byLocation(identifier1);
            if (shadertype != null) {
                loadShader(identifier1, entry.getValue(), shadertype, map, builder);
            }
        }

        Builder<Identifier, PostChainConfig> builder1 = ImmutableMap.builder();
        Map<Identifier, Resource> map1 = POST_CHAIN_ID_CONVERTER.listMatchingResources(p_363890_);

        for (int k = 0; k < astring.length; k++) {
            String s3 = astring[k];
            String s4 = "post_effect/" + s3 + ".json";
            Identifier identifier2 = Identifier.withDefaultNamespace(s4);
            Optional<Resource> optional1 = Config.getResourceSafe(identifier2);
            if (optional1.isEmpty()) {
                Config.warn("Resource not found: " + identifier2);
            } else {
                map1.put(identifier2, optional1.get());
            }
        }

        for (Entry<Identifier, Resource> entry1 : map1.entrySet()) {
    ...
    private static GlslPreprocessor createPreprocessor(final Map<Identifier, Resource> p_367930_, Identifier p_450691_) {
        ...
        return new GlslPreprocessor() {
            ...
            public  String applyImport(boolean p_365562_, String p_361440_) {
                ...
                try {
                    if (p_365562_) {
                        identifier1 = identifier.withPath(path2In -> FileUtil.normalizeResourcePath(path2In + p_361440_));
                    } else {
    ...
    private static void loadPostChain(Identifier p_460217_, Resource p_365135_, Builder<Identifier, PostChainConfig> p_362996_) {
        ...
        try (Reader reader = p_365135_.openAsReader()) {
            ...
            LOGGER.error("Failed to parse post chain at {}", p_460217_, exception);
        }
    }
    ...
    protected void apply(ShaderManager.Configs p_360858_, ResourceManager p_369986_, ProfilerFiller p_364135_) {
        ...
        if (!list.isEmpty()) {
            ...
            throw new RuntimeException(
                "Failed to load required shader programs:\n" + list.stream().map(entryIn -> " - " + entryIn).collect(Collectors.joining("\n"))
            );
        }
    ...
    public  PostChain getPostChain(Identifier p_455256_, Set<Identifier> p_362698_) {
        try {
    ...
    public  String getShader(Identifier p_460967_, ShaderType p_393108_) {
    ...
    class CompilationCache implements AutoCloseable {
        ...
        public  PostChain getOrLoadPostChain(Identifier p_456111_, Set<Identifier> p_368742_) throws ShaderManager.CompilationException {
        ...
        @Override
        public void close() {
            this.postChains.values().forEach(chainIn -> chainIn.ifPresent(PostChain::close));
            this.postChains.clear();
        }

        public  String getShaderSource(Identifier p_454193_, ShaderType p_392413_) {
