...
public class CloudRenderer extends SimplePreparableReloadListener<Optional<CloudRenderer.TextureData>> implements AutoCloseable {
    ...
    public void render(int p_369834_, CloudStatus p_363277_, float p_367079_, Vec3 p_367264_, long p_455582_, float p_364211_) {
        this.render(p_369834_, p_363277_, p_367079_, p_367264_, p_455582_, p_364211_, null);
    }

    public void render(int colorIn, CloudStatus statusIn, float cloudHeightIn, Vec3 posIn, long ticksIn, float partialTicks, GpuBufferSlice fogBufferIn) {
        if (this.texture != null) {
            ...
            if (fogBufferIn != null) {
                RenderSystem.setShaderFog(fogBufferIn);
            }
            ...
            float f = (float)(cloudHeightIn - posIn.y);
            float f1 = f + 4.0F;
            ...
            float f2 = (float)(ticksIn % (this.texture.width * 400L)) + partialTicks;
            double d0 = posIn.x + f2 * 0.030000001F;
            double d1 = posIn.z + 3.96F;
            ...
            boolean flag = Config.isCloudsFancy();
            ...
            if (this.quadCount != 0) {
                try (GpuBuffer.MappedView gpubuffer$mappedview1 = RenderSystem.getDevice()
                    ...
                    Std140Builder.intoBuffer(gpubuffer$mappedview1.data()).putVec4(ARGB.vector4fFromARGB32(colorIn)).putVec3(-f3, f, -f4).putVec3(12.0F, 4.0F, 12.0F);
                    RenderSystem.setShaderColor(ARGB.redFloat(colorIn), ARGB.greenFloat(colorIn), ARGB.blueFloat(colorIn), 1.0F);
                }
