...
public abstract class AbstractEndPortalRenderer<T extends TheEndPortalBlockEntity, S extends EndPortalRenderState> implements BlockEntityRenderer<T, S> {
    ...
    public void extractRenderState(T p_423846_, S p_429747_, float p_431596_, Vec3 p_428772_, ModelFeatureRenderer. CrumblingOverlay p_425644_) {
    ...
    public void submit(S p_430863_, PoseStack p_430020_, SubmitNodeCollector p_427983_, CameraRenderState p_427264_) {
        if (Config.isShaders()) {
            RenderType rendertype = RenderTypes.entitySolid(END_PORTAL_LOCATION);
            p_427983_.submitCustomGeometry(
                p_430020_,
                rendertype,
                (matrixIn, bufferIn) -> ShadersRender.renderEndPortal(
                    p_430863_.facesToShow, this.getOffsetUp(), matrixIn, bufferIn, 15728880, OverlayTexture.NO_OVERLAY
                )
            );
        } else {
            p_427983_.submitCustomGeometry(p_430020_, this.renderType(), (matrixIn, bufferIn) -> this.renderCube(p_430863_.facesToShow, matrixIn.pose(), bufferIn));
        }
    }
