...
public class Stitcher<T extends Stitcher.Entry> {
    ...
    public Stitcher(int p_118171_, int p_118172_, int p_118173_, int p_459323_) {
        ...
        this.padding = Config.isMultiTexture() ? 0 : 1 << p_118173_ << Mth.clamp(p_459323_ - 1, 0, 4);
    }
        ...
        for (Stitcher.Holder<T> holder : list) {
            if (!this.addToStorage(holder)) {
                if (LOGGER.isInfoEnabled()) {
                    ...
                    stringbuilder.append("Unable to fit: ").append(holder.entry().name());
                    stringbuilder.append(" - size: ").append(holder.entry.width()).append("x").append(holder.entry.height());
                    stringbuilder.append(" - Maybe try a lower resolution resourcepack?\n");
                    list.forEach(h -> stringbuilder.append("\t").append(h).append("\n"));
