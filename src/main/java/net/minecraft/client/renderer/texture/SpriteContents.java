...
public class SpriteContents implements Stitcher.Entry, AutoCloseable {
    ...
    private Optional<AnimationMetadataSection> animMetadata;
    ...
    public SpriteContents(
        ...
        this.animatedTexture = p_428820_.<SpriteContents.AnimatedTexture>map(
                metaSecIn -> this.createAnimatedTexture(p_251031_, p_252131_.getWidth(), p_252131_.getHeight(), metaSecIn)
            )
            .orElse(null);
        ...
        this.animMetadata = p_428820_;
        this.textureMetadata = p_460315_;
    }
    ...
    public void increaseMipLevel(int p_248864_) {
        ...
        if (this.sprite != null) {
            ...
            if (this.sprite.spriteShadersType == null && icolorblender == null && this.name.getPath().endsWith("glass_pane_top")) {
                icolorblender = new ColorBlenderKeepAlpha();
            }
        }

        try {
            this.byMipLevel = MipmapGenerator.generateMipLevels(this.name, this.byMipLevel, p_248864_, this.mipmapStrategy, this.alphaCutoffBias, icolorblender);
            if (this.sprite.spriteSingle != null) {
                this.sprite.spriteSingle.contents().byMipLevel = this.byMipLevel;
            }
    ...
    public SpriteContents. AnimationState createAnimationState(GpuBufferSlice p_454298_, int p_459047_) {
    ...
    public void rescale() {
        if (this.scaleFactor > 1.0) {
            int i = (int)Math.round(this.originalImage.getWidth() * this.scaleFactor);
    ...
    public Optional<AnimationMetadataSection> getAnimMetadata() {
        return this.animMetadata;
    }

    public List<MetadataSectionType.WithValue<?>> getAdditionalMetadata() {
        return this.additionalMetadata;
    }

    public Optional<TextureMetadataSection> getTextureMetadata() {
    ...
    public class AnimationState implements AutoCloseable {
        ...
        public void tick() {
            if (this.sprite != null && this.sprite.parentSprite != null && this.sprite.usesParentAnimationTime) {
                SpriteContents.AnimationState spritecontents$animationstate = this.sprite.parentSprite.getSpriteContentsTicker();
                this.frame = spritecontents$animationstate.frame;
                this.subFrame = spritecontents$animationstate.subFrame;
                this.isDirty = spritecontents$animationstate.isDirty;
            } else {
                this.subFrame++;
                this.isDirty = false;
                ...
                if (this.sprite != null) {
                    if (this.sprite.spriteSingle != null && this.sprite.spriteSingle.getSpriteContentsTicker() != null) {
                        this.sprite.spriteSingle.getSpriteContentsTicker().tick();
                    }

                    if (this.sprite.spriteNormal != null && this.sprite.spriteNormal.getSpriteContentsTicker() != null) {
                        this.sprite.spriteNormal.getSpriteContentsTicker().tick();
                    }

                    if (this.sprite.spriteSpecular != null && this.sprite.spriteSpecular.getSpriteContentsTicker() != null) {
                        this.sprite.spriteSpecular.getSpriteContentsTicker().tick();
                    }
                }
            }
        }
        ...
        public void setSpriteUbosByMip(GpuBufferSlice[] ubosByMip) {
            for (int i = 0; i < this.spriteUbosByMip.length; i++) {
                this.spriteUbosByMip[i] = ubosByMip[i];
            }
        }
