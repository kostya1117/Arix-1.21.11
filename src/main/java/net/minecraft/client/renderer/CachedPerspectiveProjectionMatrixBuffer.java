...
public class CachedPerspectiveProjectionMatrixBuffer implements AutoCloseable {
    ...
    public GpuBufferSlice getBuffer(int p_406375_, int p_407827_, float p_408594_) {
        if (this.width != p_406375_ || this.height != p_407827_ || this.fov != p_408594_ || this.shaders != Config.isShaders()) {
            ...
            this.bufferSlice.setData(matrix4f);
        }
    ...
    private Matrix4f createProjectionMatrix(int p_406878_, int p_407251_, float p_408735_) {
        ...
        if (Config.isShaders() && this == Config.getGameRenderer().hud3dProjectionMatrixBuffer) {
            Shaders.applyHandDepth(matrix4f);
        }

        return matrix4f.perspective(p_408735_ * (float) (Math.PI / 180.0), (float)p_406878_ / p_407251_, this.zNear, this.zFar);
    }
