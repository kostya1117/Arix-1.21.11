...
public class WingsLayer<S extends HumanoidRenderState, M extends EntityModel<S>> extends RenderLayer<S, M> {
    ...
    protected static @Nullable Identifier getPlayerElytraTexture(HumanoidRenderState p_364125_) {
        if (Config.isCustomItems()) {
            Identifier identifier = CustomItems.getCustomElytraTexture(p_364125_.chestEquipment, null);
            if (identifier != null) {
                return identifier;
            }
        }
        ...
        if (p_364125_ instanceof AvatarRenderState avatarrenderstate) {
            ...
            if (p_364125_.entity instanceof AbstractClientPlayer abstractclientplayer) {
                Identifier identifier1 = abstractclientplayer.getLocationElytra();
                if (identifier1 != null) {
                    return identifier1;
                }
            }

            if (Config.isShowCapes() && playerskin.elytra() != null) {
                return playerskin.elytra().texturePath();
            }

            if (Config.isShowCapes() && playerskin.cape() != null && avatarrenderstate.showCape) {
