    ...
    public ModelPart bake(int p_171584_, int p_171585_) {
        Object2ObjectArrayMap<String, ModelPart> object2objectarraymap = this.children
            ...
            .collect(
                Collectors.toMap(
                    Entry::getKey,
                    entryIn -> ((PartDefinition)entryIn.getValue()).bake(p_171584_, p_171585_),
                    (partIn, part2In) -> partIn,
                    Object2ObjectArrayMap::new
                )
            );
        List<ModelPart.Cube> list = this.cubes.stream().map(cubeDefIn -> cubeDefIn.bake(p_171584_, p_171585_)).toList();
