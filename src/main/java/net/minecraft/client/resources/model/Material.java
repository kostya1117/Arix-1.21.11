...
public class Material {
    ...
    public VertexConsumer buffer(MaterialSet p_428142_, MultiBufferSource p_119198_, Function<Identifier, RenderType> p_119199_) {
        TextureAtlasSprite textureatlassprite = p_428142_.get(this);
        textureatlassprite = TextureUtils.getCustomSprite(textureatlassprite);
        RenderType rendertype = this.renderType(p_119199_);
        if (textureatlassprite.isSpriteEmissive && rendertype.isEntitySolid()) {
            RenderUtils.flushRenderBuffers();
            rendertype = RenderTypes.entityCutout(this.atlasLocation);
        }

        return textureatlassprite.wrap(p_119198_.getBuffer(rendertype));
    }
