...
public class RecipeManager extends SimplePreparableReloadListener<RecipeMap> implements RecipeAccess {
                ...
                p_359840_ -> {
                    ...
                    if (!recipe.isSpecial() && recipe.placementInfo().isImpossibleToPlace()) {
                        ...
                        if (recipe instanceof StonecutterRecipe stonecutterrecipe) {
                            RecipeHolder<StonecutterRecipe> recipeholder = (RecipeHolder<StonecutterRecipe>)p_359840_;
                            if (isIngredientEnabled(p_360842_, stonecutterrecipe.input()) && stonecutterrecipe.resultDisplay().isEnabled(p_360842_)) {
                                list.add(
                                    new SelectableRecipe.SingleInputEntry<>(
                                        stonecutterrecipe.input(), new SelectableRecipe<>(stonecutterrecipe.resultDisplay(), Optional.of(recipeholder))
                                    )
                                );
                            }
                        }
    ...
    public <I extends RecipeInput, T extends Recipe<I>> Optional<RecipeHolder<T>> getRecipeFor(
        RecipeType<T> p_220249_, I p_344518_, Level p_220251_,  ResourceKey<Recipe<?>> p_361142_
    ) {
    ...
    public <I extends RecipeInput, T extends Recipe<I>> Optional<RecipeHolder<T>> getRecipeFor(
        RecipeType<T> p_343647_, I p_342793_, Level p_344483_,  RecipeHolder<T> p_345187_
    ) {
