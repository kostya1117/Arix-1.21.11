...
public class ChunkMap extends SimpleRegionStorage implements ChunkHolder.PlayerProvider, GeneratingChunkMap {
    ...
    private CompletableFuture<ChunkAccess> scheduleChunkLoad(ChunkPos p_140418_) {
        CompletableFuture<Optional<SerializableChunkData>> completablefuture = this.readChunk(p_140418_).thenApplyAsync(optTagIn -> optTagIn.map(tag2In -> {
            SerializableChunkData serializablechunkdata = SerializableChunkData.parse(this.level, this.level.palettedContainerFactory(), tag2In);
        ...
        return completablefuture.<Object, Optional<SerializableChunkData>>thenCombine(
            ...
            .thenApplyAsync(optDataIn -> {
                ...
                if (optDataIn.isPresent()) {
                    ChunkAccess chunkaccess = optDataIn.get().read(this.level, this.poiManager, this.storageInfo(), p_140418_);
            ...
            .exceptionallyAsync(throwableIn -> this.handleChunkLoadFailure(throwableIn, p_140418_), this.mainThreadExecutor);
    }
    ...
    public CompletableFuture<ChunkResult<LevelChunk>> prepareTickingChunk(ChunkHolder p_143054_) {
        ...
        return completablefuture.thenApplyAsync(chunkResIn -> chunkResIn.map(listIn -> {
            ...
            if (completablefuture1.isDone()) {
                ...
                completablefuture1.thenAcceptAsync(voidIn -> this.onChunkReadyToSend(p_143054_, levelchunk), this.mainThreadExecutor);
            }
    ...
    Stream<ChunkHolder> allChunksWithAtLeastStatus(ChunkStatus p_425975_) {
        ...
        return this.visibleChunkMap.values().stream().filter(holder2In -> holder2In.getTicketLevel() <= i);
    }
    ...
    private boolean save(ChunkAccess p_140259_) {
        ...
        try {
            ...
            if (Reflector.ForgeEventFactory_onChunkDataSave.exists() && Reflector.ChunkAccess_getWorldForge.exists()) {
                Level level = (Level)Reflector.call(p_140259_, Reflector.ChunkAccess_getWorldForge);
                Reflector.ForgeEventFactory_onChunkDataSave.call(p_140259_, level != null ? level : this.level, serializablechunkdata);
            }
    ...
    private CompletableFuture<Optional<CompoundTag>> readChunk(ChunkPos p_214964_) {
        return this.read(p_214964_).thenApplyAsync(tagIn -> tagIn.map(this::upgradeChunkTag), Util.backgroundExecutor().forName("upgradeChunk"));
    }
    ...
    public static CompoundTag getChunkDataFixContextTag(ResourceKey<Level> p_455737_, Optional<ResourceKey<MapCodec<? extends ChunkGenerator>>> p_460440_) {
        ...
        p_460440_.ifPresent(key2In -> compoundtag.putString("generator", key2In.identifier().toString()));
        return compoundtag;
    }
    ...
    void forEachBlockTickingChunk(Consumer<LevelChunk> p_394956_) {
        this.distanceManager.forEachEntityTickingChunk(chunkPosIn -> {
            ChunkHolder chunkholder = this.visibleChunkMap.get(chunkPosIn);
            if (chunkholder != null) {
                LevelChunk levelchunk = chunkholder.getTickingChunk();
                if (levelchunk != null) {
