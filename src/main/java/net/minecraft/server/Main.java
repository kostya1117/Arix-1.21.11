        ...
        try {
            ...
            if (levelstoragesource$levelstorageaccess.hasWorldData()) {
                ...
                try {
                    ...
                    LOGGER.warn("Failed to load world data from {}", levelstoragesource$leveldirectory.dataFile(), exception2);
                    LOGGER.info("Attempting to use fallback");

                    try {
                        dynamic = levelstoragesource$levelstorageaccess.getDataTagFallback();
                        levelsummary = levelstoragesource$levelstorageaccess.getSummary(dynamic);
                    } catch (IOException | NbtException | ReportedNbtException exception1) {
                        LOGGER.error("Failed to load world data from {}", levelstoragesource$leveldirectory.oldDataFile(), exception1);
                        LOGGER.error(
