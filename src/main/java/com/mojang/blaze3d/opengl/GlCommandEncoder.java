...
import net.optifine.render.RegionRenderer;
...
public class GlCommandEncoder implements CommandEncoder {
    ...
    private boolean mainFramebufferEnabled = Config.isMainFramebufferEnabled();
    private boolean prevFlushRenderPass;
    ...
    protected GlCommandEncoder(GlDevice p_396674_) {
        ...
        this.device.debugLabels().applyLabelFramebuffer(this.readFbo, "GLCE Read");
        this.device.debugLabels().applyLabelFramebuffer(this.drawFbo, "GLCE Draw");
    }
    ...
    public RenderPass createRenderPass(
        ...
        this.prevFlushRenderPass = RenderUtils.setFlushRenderBuffers(false);
        this.device.debugLabels().pushDebugGroup(p_406861_);
        if (this.isBindFramebuffer(p_408235_, p_409576_)) {
        ...
        if (p_391728_.isPresent()) {
            int j = p_391728_.getAsInt();
            GL11.glClearColor(ARGB.redFloat(j), ARGB.greenFloat(j), ARGB.blueFloat(j), ARGB.alphaFloat(j));
            k |= 16384;
        }
    ...
    public void clearColorTexture(GpuTexture p_394273_, int p_393834_) {
        ...
        if (this.isBindFramebuffer(p_394273_, null)) {
    ...
    public void clearColorAndDepthTextures(GpuTexture p_393527_, int p_391700_, GpuTexture p_391582_, double p_393930_) {
        ...
        if (this.isBindFramebuffer(p_393527_, p_391582_)) {
    ...
    public void clearColorAndDepthTextures(
        ...
        if (this.isBindFramebuffer(p_409650_, p_409914_)) {
    ...
    public void clearDepthTexture(GpuTexture p_397307_, double p_397388_) {
        ...
        if (this.isBindFramebuffer(null, p_397307_)) {
    ...
    public void copyTextureToBuffer(
        ...
        if (p_395739_ >= 0 && p_395739_ < p_391871_.getMipLevels()) {
            ...
            if (p_391264_ + p_396780_ > p_391871_.getWidth(p_395739_) || p_393748_ + p_391271_ > p_391871_.getHeight(p_395739_)) {
                throw new IllegalArgumentException(
                    "Copy source texture ("
    ...
    public void copyTextureToTexture(
        ...
        if (p_396176_ >= 0 && p_396176_ < p_394155_.getMipLevels() && p_396176_ < p_394461_.getMipLevels()) {
            ...
            this.device.directStateAccess().bindFrameBufferTextures(this.readFbo, flag ? 0 : i, flag ? i : 0, p_396176_, 0);
            this.device.directStateAccess().bindFrameBufferTextures(this.drawFbo, flag ? 0 : j, flag ? j : 0, p_396176_, 0);
            this.device
                .directStateAccess()
                .blitFrameBuffers(
                    this.readFbo,
    ...
    public void presentTexture(GpuTextureView p_406941_) {
        if (this.isBindFramebuffer(p_406941_, null)) {
            if (this.inRenderPass) {
                throw new IllegalStateException("Close the existing render pass before performing additional commands");
            }
    ...
    protected <T> void executeDrawMultiple(
        ...
        if (this.trySetup(p_396459_, p_407124_)) {
            ...
            for (RenderPass.Draw<T> draw : p_398042_) {
                ...
                if (biconsumer != null) {
                    biconsumer.accept(p_409520_, (p_404740_, p_404741_) -> {
                        if (Config.isShaders() && Shaders.activeProgramID > 0) {
                            if (p_404740_.equals("DynamicTransforms")) {
                                Shaders.setDynamicTransforms(p_404741_);
                            } else if (p_404740_.equals("ChunkSection")) {
                                Shaders.setChunkSectionInfo(p_404741_);
                            }
                        }
                ...
                if (draw.regionRenderData() != null) {
                    this.drawRegion(p_396459_, draw.regionRenderData());
                } else if (draw.multiTextureData() != null) {
                    this.drawMultiTexture(p_396459_, vertexformat$indextype, draw.multiTextureData());
                } else {
    ...
    private void drawRegion(GlRenderPass passIn, RegionRenderData dataIn) {
        RegionRenderer regionrenderer = dataIn.getRenderer();
        PairInt pairint = dataIn.getPosition();
        regionrenderer.renderRegion(pairint);
    }

    private void drawMultiTexture(GlRenderPass passIn, VertexFormat.IndexType indexTypeIn, MultiTextureData multiTextureData) {
        RenderPipeline renderpipeline = passIn.pipeline.info();
        this.device.vertexArrayCache().bindVertexArray(renderpipeline.getVertexFormat(), (GlBuffer)passIn.vertexBuffers[0]);
        if (indexTypeIn != null) {
            GlStateManager._glBindBuffer(34963, ((GlBuffer)passIn.indexBuffer).handle);
        }

        MultiTextureRenderer.draw(renderpipeline.getVertexFormatMode(), GlConst.toGl(indexTypeIn), multiTextureData);
    }
    ...
    private boolean trySetup(GlRenderPass p_396081_, Collection<String> p_408722_) {
        ...
        if (this.lastProgram != null && this.lastProgram.getProgramId() != GlStateManager.getProgram()) {
            this.lastProgram = null;
        }

        boolean flag2 = this.lastProgram != glprogram;
        if (flag2) {
        ...
        for (Entry<String, Uniform> entry1 : glprogram.getUniforms().entrySet()) {
            ...
            switch ((Uniform)entry1.getValue()) {
                case Uniform.Ubo(int i):
                    int i2 = i;
                    if (flag) {
                        GpuBufferSlice gpubufferslice2 = p_396081_.uniforms.get(s);
                        GL32.glBindBufferRange(35345, i2, ((GlBuffer)gpubufferslice2.buffer()).handle, gpubufferslice2.offset(), gpubufferslice2.length());
                    }
                    break;
                case Uniform.Utb(int j, int k, TextureFormat textureformat, int l):
                    int j2 = l;
                    if (flag2 || flag) {
                    ...
                    GL11C.glBindTexture(35882, j2);
                    if (flag) {
                        GpuBufferSlice gpubufferslice3 = p_396081_.uniforms.get(s);
                        GL31.glTexBuffer(35882, GlConst.toGlInternalId(textureformat), ((GlBuffer)gpubufferslice3.buffer()).handle);
                    }
                    break;
                case Uniform.Sampler(int i1, int j1):
                    int k2 = j1;
                    ...
                    int k1 = this.getTextureUnit(s, k2);
                    if (flag2 || flag) {
                        GlStateManager._glUniform1i(i1, k1);
                    }
                    ...
                    if ((gltexture.usage() & 16) != 0) {
                        ...
                        l1 = 3553;
                        if (k1 == 1 && Shaders.activeProgramID > 0 && Shaders.isOverlayDisabled()) {
                            break;
                        }

                        flag1 = true;
                        if (Config.isShaders()) {
                            ShadersTex.bindTexture(gltexture);
                        } else {
                            GlStateManager._bindTexture(gltexture.id);
                        }
                    }

                    if (!flag1 || !Config.isShaders() && !Config.isMultiTexture()) {
                        GlStateManager.bindSampler(k1, glrenderpass$textureviewandsampler1.sampler().getId());
                    } else {
                        gltexture.applySampler(glrenderpass$textureviewandsampler1.sampler());
                        GlStateManager.bindSampler(k1, 0);
                    }

                    GlStateManager._texParameter(l1, 33084, gltextureview1.baseMipLevel());
                    GlStateManager._texParameter(l1, 33085, gltextureview1.baseMipLevel() + gltextureview1.mipLevels() - 1);
                    break;
                default:
        ...
        if (Config.isShaders() && Shaders.activeProgramID > 0) {
            ...
            for (Entry<String, GpuBufferSlice> entry2 : p_396081_.uniforms.entrySet()) {
                if (entry2.getKey().equals("DynamicTransforms")) {
                    GpuBufferSlice gpubufferslice1 = entry2.getValue();
                    Shaders.setDynamicTransforms(gpubufferslice1);
                }
            }
        }
    ...
    public void finishRenderPass() {
        ...
        RenderUtils.setFlushRenderBuffers(this.prevFlushRenderPass);
        GlStateManager._glBindFramebuffer(36160, 0);
    ...
    private boolean isBindFramebuffer(GpuTextureView colorTextureIn, GpuTextureView depthTextureIn) {
        GpuTexture gputexture = colorTextureIn != null ? colorTextureIn.texture() : null;
        GpuTexture gputexture1 = depthTextureIn != null ? depthTextureIn.texture() : null;
        return this.isBindFramebuffer(gputexture, gputexture1);
    }

    private boolean isBindFramebuffer(GpuTexture colorTextureIn, GpuTexture depthTextureIn) {
        if (this.mainFramebufferEnabled) {
            return true;
        }

        RenderTarget rendertarget = Minecraft.getInstance().getMainRenderTarget();
        return colorTextureIn == rendertarget.getColorTexture() ? false : depthTextureIn != rendertarget.getDepthTexture();
    }

    public void useVanillaProgram() {
        if (this.lastProgram != null) {
            GlStateManager._glUseProgram(this.lastProgram.getProgramId());
        }
    }

    public void bindFramebuffer(RenderTarget targetIn) {
        GlTexture gltexture = (GlTexture)targetIn.getColorTexture();
        GlTexture gltexture1 = (GlTexture)targetIn.getDepthTexture();
        int i = gltexture.getFbo(this.device.directStateAccess(), gltexture1);
        GlStateManager._glBindFramebuffer(36160, i);
    }
