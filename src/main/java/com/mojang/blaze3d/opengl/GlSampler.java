...
public class GlSampler extends GpuSampler {
    ...
    public boolean matches(GlSampler sampler) {
        if (sampler == null) {
            return false;
        } else if (this.id == sampler.getId() && this.addressModeU != sampler.getAddressModeU()) {
            return false;
        } else if (this.addressModeV != sampler.getAddressModeV()) {
            return false;
        } else if (this.minFilter != sampler.getMinFilter()) {
            return false;
        } else if (this.magFilter != sampler.getMagFilter()) {
            return false;
        } else {
            return this.maxAnisotropy != sampler.getMaxAnisotropy() ? false : this.maxLod.equals(sampler.getMaxLod());
        }
    }
