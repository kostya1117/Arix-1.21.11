...
public class BufferBuilder implements VertexConsumer {
    ...
    private final ChunkSectionLayer blockLayer;
    ...
    public BufferBuilder(ByteBufferBuilder p_342927_, VertexFormat.Mode p_344709_, VertexFormat p_342329_) {
        this(p_342927_, p_344709_, p_342329_, null, null);
    }

    public BufferBuilder(
        ByteBufferBuilder byteBufferIn, VertexFormat.Mode drawModeIn, VertexFormat vertexFormatIn, RenderType renderTypeIn, ChunkSectionLayer blockLayerIn
    ) {
    ...
    private @Nullable MeshData storeMesh() {
        ...
        int j = this.vertices;
        if (Config.isRenderRegions() && RegionLayerRenderer.canRenderLayer(this.blockLayer)) {
            j = Math.max(this.vertices, 65536);
        }

        VertexFormat.IndexType vertexformat$indextype = VertexFormat.IndexType.least(j);
        MultiTextureData multitexturedata = this.multiTextureBuilder.build(this.vertices, this.blockLayer, this.quadSprites, null);
        return new MeshData(
    ...
    @Override
    public ChunkSectionLayer getBlockLayer() {
    ...
    public String toString() {
        String s = this.renderType != null
            ? "renderType: " + this.renderType.getName()
            : (this.blockLayer != null ? "blockLayer: " + this.blockLayer.name() : "?");
        return s
            + ", vertexFormat: "
            + this.format.getName()
            + ", vertexSize: "
