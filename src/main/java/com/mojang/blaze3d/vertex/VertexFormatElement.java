...
@DontObfuscate
public record VertexFormatElement(int id, int index, VertexFormatElement.Type type, VertexFormatElement.Usage usage, int count, String name, int attributeIndex) {
    ...
    public static final VertexFormatElement LINE_WIDTH = register(
        6, 0, VertexFormatElement.Type.FLOAT, VertexFormatElement.Usage.GENERIC, 1, "LINE_WIDTH_1F", 6
    );
    ...
    public static VertexFormatElement register(
        ...
        if (vertexformatelement.name == null) {
            Log.warn("Invalid element name: " + vertexformatelement.name + ", element: " + vertexformatelement);
        }

        for (VertexFormatElement vertexformatelement1 : ELEMENTS) {
            if (Config.equals(vertexformatelement.name, vertexformatelement1.name)) {
                Log.warn("Duplicate element name: " + vertexformatelement.name + ", element: " + vertexformatelement);
            }

            if (vertexformatelement.attributeIndex >= 0 && vertexformatelement.attributeIndex == vertexformatelement1.attributeIndex) {
                Log.warn("Duplicate attribute index: " + vertexformatelement.attributeIndex + ", element: " + vertexformatelement);
            }
        }
