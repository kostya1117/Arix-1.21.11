...
public final class NativeImage implements AutoCloseable {
    ...
    public static NativeImage read(byte[] p_273041_) throws IOException {
        ...
        if (i < p_273041_.length) {
            ...
            NativeImage nativeimage;
            try {
                nativeimage = putAndRead(bytebuffer1, p_273041_);
            } finally {
            ...
            try (MemoryStack memorystack1 = MemoryStack.stackPush()) {
                ...
                return putAndRead(bytebuffer, p_273041_);
            }
        }
    }
    ...
    public static NativeImage read(NativeImage. Format p_85052_, ByteBuffer p_85053_) throws IOException {
        ...
        if (MemoryUtil.memAddress(p_85053_) == 0L) {
    ...
    public int getPixelABGR(int p_366605_, int p_368577_) {
        ...
        if (this.isOutsideBounds(p_366605_, p_368577_)) {
    ...
    public void setPixelABGR(int p_366486_, int p_360988_, int p_364498_) {
        ...
        if (this.isOutsideBounds(p_366486_, p_360988_)) {
            throw new IllegalArgumentException(
    ...
    public byte getLuminanceOrAlpha(int p_85088_, int p_85089_) {
        ...
        if (this.isOutsideBounds(p_85088_, p_85089_)) {
            throw new IllegalArgumentException(
    ...
    public void copyFrom(NativeImage p_85055_) {
        ...
        if (this.width == p_85055_.width) {
            ...
            for (int l = 0; l < k; l++) {
                ...
                MemoryUtil.memCopy(p_85055_.pixels + i1, this.pixels + j1, j * i);
            }
        }
    }
    ...
    public void downloadFromFramebuffer() {
        this.checkAllocated();
        this.format.setGlPackAlignment();
        GlStateManager._readPixels(0, 0, this.width, this.height, GlConst.toGl(this.format), 5121, this.pixels);
    }
    ...
    public void uploadTextureSub(
        ...
        this.uploadTextureSub(level, xOffset, yOffset, unpackSkipPixels, unpackSkipRows, widthIn, heightIn, autoClose);
        setMinMagFilters(blur, mipmap);
        setClamp(clamp);
    }

    public void uploadTextureSub(int level, int xOffset, int yOffset, boolean autoClose) {
        this.uploadTextureSub(level, xOffset, yOffset, 0, 0, this.width, this.height, autoClose);
    }

    public void uploadTextureSub(int level, int xOffset, int yOffset, int unpackSkipPixels, int unpackSkipRows, int widthIn, int heightIn, boolean autoClose) {
        RenderSystem.assertOnRenderThread();
        this.uploadTextureSubRaw(level, xOffset, yOffset, unpackSkipPixels, unpackSkipRows, widthIn, heightIn, autoClose);
    }

    private void uploadTextureSubRaw(
        int level, int xOffset, int yOffset, int unpackSkipPixels, int unpackSkipRows, int widthIn, int heightIn, boolean autoClose
    ) {
        try {
            RenderSystem.assertOnRenderThread();
            this.checkAllocated();
            ...
            this.format.setGlUnpackAlignment();
            GlStateManager._texSubImage2D(3553, level, xOffset, yOffset, widthIn, heightIn, GlConst.toGl(this.format), 5121, this.pixels);
        } finally {
            if (autoClose) {
                this.close();
            }
