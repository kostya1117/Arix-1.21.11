...
public final class TextSettingComponent implements IComponent {
    ...
    public void render(GuiGraphics guiGraphics, float x, float y, float width,
        ...
        if (hovered(mouseX, mouseY, fieldX, y, FIELD_W, FIELD_H)) {
        ...
        if (input.isEmpty()) {
            FontManager.get(10).drawString(guiGraphics, "Печатайте : ", textX, textY, ColorUtil.replAlpha(textInactive, (int)(ColorUtil.alpha(textInactive) * 0.5f)));
        } else {
            float curX = textX;
