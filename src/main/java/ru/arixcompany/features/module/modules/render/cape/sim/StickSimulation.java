...
public class StickSimulation implements BasicSimulation {
    ...
    @Override
    public boolean empty() { return sticks.isEmpty(); }
    ...
    public static class Point implements CapePoint {
        ...
        public float getLerpX(float delta) { return Mth.lerp(delta, prevPosition.x, position.x); }
        public float getLerpY(float delta) { return Mth.lerp(delta, prevPosition.y, position.y); }
        public float getLerpZ(float delta) { return 0; }
    }
    ...
    public static class Vector2 {
        ...
        public void copy(Vector2 vec) { this.x = vec.x; this.y = vec.y; }
        public Vector2 add(Vector2 vec) { this.x += vec.x; this.y += vec.y; return this; }
        public Vector2 subtract(Vector2 vec) { this.x -= vec.x; this.y -= vec.y; return this; }
        public Vector2 div(float amount) { this.x /= amount; this.y /= amount; return this; }
        public Vector2 mul(float amount) { this.x *= amount; this.y *= amount; return this; }

        public Vector2 normalize() {
            float f = Mth.sqrt(this.x * this.x + this.y * this.y);
            if (f < 1.0E-4F) { this.x = 0; this.y = 0; }
            else { this.x /= f; this.y /= f; }
            return this;
        }
