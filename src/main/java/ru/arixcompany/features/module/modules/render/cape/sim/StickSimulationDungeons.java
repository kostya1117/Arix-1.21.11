...
public class StickSimulationDungeons implements BasicSimulation {
            ...
            for (int a = 0; a < points.size(); a++) {
                for (int b = a + 1; b < points.size(); b++) {
                    ...
                    if (stickDir.sqrMagnitude() < 0.99) {
                        ...
                        if (!pA.locked) pA.position = centre.clone().add(stickDir.clone().mul(1f / 2f));
                        if (!pB.locked) pB.position = centre.clone().subtract(stickDir.clone().mul(1f / 2f));
                    }
                }
            }
    ...
    private void preventHardBends() {
        for (int i = 1; i < points.size() - 1; i++) {
            ...
            float mb = maxBend;
            if (i != points.size() / 2) mb = 0;
            if (angle < -mb) {
                Vector3 replacement = getReplacement(points.get(i).position, points.get(i - 1).position, -mb * 2);
    ...
    public static class Point implements CapePoint {
        ...
        @Override
        public float getLerpZ(float delta) { return Mth.lerp(delta, prevPosition.z, position.z); }
    }
