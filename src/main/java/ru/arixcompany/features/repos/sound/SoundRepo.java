...
public final class SoundRepo {
    ...
    private static void playInternal(String fileName, float volume, float pitch) {
        ...
        try (InputStream resource = SoundRepo.class.getClassLoader().getResourceAsStream(path)) {
            if (resource == null) {
                MessageSender.error("Sound not found: " + path);
                return;
            }
            ...
            MessageSender.error("Failed to play: " + fileName);
            e.printStackTrace();
        }
    }
    ...
    public static float getClientVolume() {
        ClientSounds clientSounds = (ClientSounds) Arix.getInstance().getModuleRepo().getModule(ClientSounds.class);
